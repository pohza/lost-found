...
function ReportFoundPage() {
  ...
  const [formValues, setFormValues] = useState<FoundItemFormValues>({
    itemName: "",
    location: "",
    dateTime: "",
    description: "",
    ...
    (async () => {
      try {
        ...
        setFormValues((prev) => ({
          ...
          handoverType: data.handoverType ?? "self",
  ...
  const handleChange = (
    field: keyof FoundItemFormValues,
    ...
    if (editId) {
      ...
      try {
        const payload = {
          ...
          handoverType: formValues.handoverType,
          howToClaim: formValues.howToClaim,
        };
      ...
      <main className="report-main">
        <div className="report-form-wrapper">
          <div className="report-form-tabs">
            <Link to="/report/lost" className="report-form-tab">
              Lost Items
            </Link>
            <Link to="/report/found" className="report-form-tab active">
              Found Items
            </Link>
          </div>
          ...
          <section className="report-form-panel">
            <h1 className="report-form-title">
              {editId ? "แก้ไขประกาศของที่พบ" : "Report Found Item"}
            </h1>
            ...
            <form className="report-form" onSubmit={handleSubmit} style={{ display: loadingEdit ? "none" : undefined }}>
              ...
              <div className="report-field">
                <span className="report-field-label">ส่งมอบของ</span>
                <div className="report-radio-group">
                  <label className="report-radio-label">
                    <input
                      ...
                      name="handoverType"
                      value="center"
                      checked={formValues.handoverType === "center"}
                      onChange={() => handleChange("handoverType", "center")}
                    />
                    <span>ส่งมอบให้จุดรับของกลาง</span>
                  </label>
                  <label className="report-radio-label">
                    <input
                      type="radio"
                      name="handoverType"
                      value="self"
                      checked={formValues.handoverType === "self"}
                      onChange={() => handleChange("handoverType", "self")}
                    />
                    <span>เก็บไว้เองชั่วคราว</span>
                  </label>
                </div>
              </div>

              <label className="report-field">
                <span className="report-field-label">วิธีรับคืน</span>
                <textarea
                  className="report-textarea"
                  placeholder="ระบุวิธีที่เจ้าของสามารถติดต่อรับคืนได้ (เช่น นัดรับที่จุดรับของกลาง, ติดต่อทางแชทในระบบ)"
                  value={formValues.howToClaim}
                  onChange={(e) =>
                    handleChange("howToClaim", e.target.value)
                  }
                  rows={2}
                />
              </label>
